...
#                         self.action['move'] = next_pos
#                     else:
#                         neighbors = env.get_neighborhood(self.current_position)
#                         passable_neighbors = [pos for pos in neighbors if env.is_passable(pos)]
#                         if passable_neighbors:
#                             self.action['move'] = random.choice(passable_neighbors)
#                         else:
#                             self.action['move'] = self.current_position
#                     return
#                 else:
#                     # No trash to pick up, reset flag
#                     self.was_sanctioned = False
...
#                 self.used_bin_this_trip = True  # Record that the agent used the bin
#                 return  # Only perform one action per timestep
#             elif not self.comp and (self.current_position not in env.house_positions + env.office_position+env.park_positions):
#                 self.action['action'] = 'litter'
#                 self.trash_count = 0
#                 self.has_littered_this_trip = True
#                 return  # Only perform one action per timestep
